...
export default function Employee() {
  ...
  <div className="flex-1 flex flex-col">
    ...
    <header className="flex justify-between items-center bg-gray-900/80 backdrop-blur-md px-8 py-4 border-b border-gray-800 shadow-md">
      <h1 className="text-2xl font-extrabold text-white flex items-center gap-2">
        <FiUsers size={22} className="text-indigo-400" /> Employees
      </h1>
      <button
        onClick={() => {
          resetForm();
        ...
        className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2 rounded-md transition shadow-md"
      >
    ...
    {/* Filters */}
    <div className="bg-gray-900/70 border-b border-gray-800 px-8 py-4 flex flex-wrap items-center justify-between gap-4 backdrop-blur-md">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredMembers.map((member) => (
            <div
              key={member._id}
              className="bg-gray-900/80 border border-gray-800 rounded-xl shadow-md hover:shadow-lg hover:border-indigo-600 transition p-6 flex flex-col justify-between backdrop-blur-md"
            >
              ...
              <div className="mt-4 text-sm text-gray-400 space-y-2">
                <p className="flex items-center gap-2">
                  <FiMail className="text-gray-500" /> {member.email}
                </p>
                <p className="flex items-center gap-2">
                  <FiPhone className="text-gray-500" /> {member.phone}
                </p>
              </div>
              ...
              <div className="flex justify-between items-center mt-6 pt-3 border-t border-gray-800">
                ...
                <button
                  ...
                  className="text-indigo-400 text-xs font-semibold hover:text-indigo-300 transition"
                >
                  View Details →
                </button>
              </div>
  ...
  {/* Drawer Form */}
  <div
    className={`fixed top-0 right-0 h-full w-full sm:w-[450px] bg-gray-900/95 backdrop-blur-md border-l border-gray-800 shadow-2xl transform transition-transform duration-300 ease-in-out z-50 ${
  ...
  >
    ...
    >
      ...
      <div>
        ...
        <div className="border-2 border-dashed border-gray-700 rounded-lg p-4 bg-gray-800/50 flex flex-col items-center justify-center relative hover:border-indigo-500 transition">
          {newEmployee.avatar ? (
            <>
              <img
                src={
                  typeof newEmployee.avatar === "string"
                    ? newEmployee.avatar
                    : URL.createObjectURL(newEmployee.avatar)
                }
                alt="Employee"
              ...
              <button
                ...
                className="absolute top-2 right-2 bg-red-500 text-white text-xs px-1.5 py-0.5 rounded-full hover:bg-red-600"
              >
                ✕
              </button>
            </>
      ...
      <div>
        ...
        <label
          htmlFor="documents"
          className="flex flex-col items-center justify-center border-2 border-dashed border-gray-700 rounded-lg p-4 cursor-pointer hover:border-indigo-500 transition bg-gray-800/50"
        >
        ...
        {newEmployee.documents.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-3">
            {newEmployee.documents.map((file, index) => (
              <span
                key={index}
                className="flex items-center gap-2 bg-indigo-600/20 border border-indigo-500/30 text-indigo-300 text-xs px-2 py-1 rounded-full"
              >
                {file.name || file}
                <button
                  type="button"
      ...
      <div className="flex justify-end gap-3 pt-6">
        <button
          ...
          className="px-4 py-2 border border-gray-700 rounded-md text-sm text-gray-400 hover:bg-gray-800"
        >
          Cancel
        </button>
        <button
          type="submit"
          className="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm hover:bg-indigo-700 shadow-md"
        >
          Save Employee
        </button>
      </div>
  ...
  {/* Delete Modal */}
  {showDeleteModal && (
    <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-md flex items-center justify-center z-[60] animate-fadeIn">
