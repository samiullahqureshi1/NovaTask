  ...
  return (
    <div className="flex h-screen bg-gradient-to-b from-gray-950 to-gray-900 text-gray-200 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col">
        ...
        {/* FILTERS */}
        <div className="flex items-center justify-between px-8 py-4 bg-gray-900/70 border-b border-gray-800 backdrop-blur-md">
          <div className="flex items-center bg-gray-800 rounded-md px-3 py-2 w-full max-w-sm">
            <FiSearch className="text-gray-400" size={18} />
            <input
              type="text"
              placeholder="Search projects..."
              className="bg-transparent outline-none ml-2 text-sm text-gray-300 w-full"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
          </div>
          <div className="flex items-center gap-3">
            ...
            <select
              className="bg-gray-800 border border-gray-700 rounded-md text-sm px-3 py-1.5 text-gray-300 focus:ring-2 focus:ring-indigo-500 outline-none"
              value={filter}
              onChange={(e) => setFilter(e.target.value)}
            >
              <option>All</option>
          ...
          {filteredProjects.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredProjects.map((project) => (
                <div
                  key={project._id}
                  className="bg-gray-900/70 border border-gray-800 rounded-xl shadow-md hover:shadow-lg hover:shadow-indigo-500/10 transition p-6 flex flex-col justify-between backdrop-blur-md"
                >
                  <div className="flex justify-between items-start mb-4">
                    <div>
                  ...
                  <div className="flex items-center justify-between mt-4">
                    ...
                    <div className="text-sm text-gray-400 space-y-1">
                      <p>
                        <FiFolder className="inline text-gray-500" />{" "}
                        {project.status}
                      </p>
                      <p>
                        <FiCalendar className="inline text-gray-500" />{" "}
                        {formatDate(project.deadline || project.endDate)}
                      </p>
                    </div>
                  </div>
      ...
      {/* 🧩 Project Drawer (Dark Theme) */}
<div
  className={`fixed top-0 right-0 h-full w-full sm:w-[450px] bg-gray-900/90 backdrop-blur-md border-l border-gray-800 shadow-2xl transform transition-transform duration-300 ease-in-out z-50 ${
...
>
  ...
  >
    ...
    <div>
      ...
      <label
        htmlFor="projectDocs"
        className="flex flex-col items-center justify-center border-2 border-dashed border-gray-700 rounded-lg p-6 cursor-pointer hover:border-indigo-500 transition group bg-gray-800/60"
      >
        <FiUpload
          size={28}
      ...
      {newProject.projectDocuments.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-3">
          {newProject.projectDocuments.map((file, index) => (
            <span
              key={index}
              className="flex items-center gap-2 bg-indigo-600/20 border border-indigo-500/20 text-indigo-300 text-xs px-2 py-1 rounded-full"
            >
              {file.name || file.split("/").pop()}
              <button
                type="button"
    ...
    <div className="flex justify-end gap-3 pt-6">
      ...
      <button
        type="submit"
        className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md text-sm shadow-md"
      >
        Save Project
      </button>
    </div>
...
{showDeleteModal && (
  <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[70]">
    <div className="bg-gray-900 border border-gray-800 rounded-lg shadow-2xl w-full max-w-md p-8 relative backdrop-blur-md">
      <button
        onClick={() => setShowDeleteModal(false)}
      ...
      <div className="text-center">
        <div className="flex justify-center mb-4">
          <div className="w-12 h-12 bg-red-600/20 rounded-full flex items-center justify-center">
            <FiFolder className="text-red-500" size={28} />
          </div>
        </div>
