...
export default function Files() {
  ...
  <div className="flex-1 flex flex-col">
    ...
    <header className="flex justify-between items-center bg-gray-900/90 backdrop-blur-md px-8 py-4 border-b border-gray-800 shadow-md">
      <h1 className="text-2xl font-extrabold text-white flex items-center gap-2">
        <FiFileText size={22} className="text-indigo-400" /> All Employee Files
      </h1>

      <label className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2 rounded-md cursor-pointer shadow-md transition">
        <FiUpload size={16} /> {uploading ? "Uploading..." : "Upload File"}
        <input
          type="file"
          className="hidden"
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredFiles.map((file) => (
            <div
              key={file._id}
              className="bg-gray-900/80 border border-gray-800 rounded-xl shadow-md hover:shadow-lg hover:border-indigo-500 transition-all p-6 flex flex-col justify-between backdrop-blur-md"
            >
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-3">
              ...
              <div className="flex justify-between items-center mt-5 pt-3 border-t border-gray-800">
                ...
                <button
                  onClick={() => openDeleteModal(file)}
                  disabled={deleting === file._id}
                  className="flex items-center gap-1 text-red-500 text-xs font-semibold hover:text-red-400 transition"
                >
                  <FiTrash2 size={14} />
                  {deleting === file._id ? "Deleting..." : "Delete"}
                </button>
  ...
  {fileToDelete && (
    <div className="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-md z-50 animate-fadeIn">
      <div className="bg-gray-900/95 border border-gray-800 rounded-2xl shadow-2xl p-6 w-96 text-gray-200">
        <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            onClick={confirmDelete}
            disabled={deleting === fileToDelete._id}
            className="px-4 py-2 text-sm bg-red-600 hover:bg-red-700 text-white rounded-md shadow-md transition disabled:opacity-50"
          >
            {deleting === fileToDelete._id ? "Deleting..." : "Delete"}
          </button>
        </div>
